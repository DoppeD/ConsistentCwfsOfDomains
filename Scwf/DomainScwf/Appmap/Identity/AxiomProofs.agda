...
id↦-mono {Γ = Γ} {𝑦 = 𝑦} {𝑧} 𝑥⊑𝑦 (id↦-intro _ _ 𝑧⊑𝑥)
  = id↦-intro 𝑦 𝑧 (NbhSys.⊑-trans (ValNbhSys _) 𝑧⊑𝑥 𝑥⊑𝑦)

id↦-bottom : 𝑥 id↦ ⊥ᵥ
id↦-bottom {𝑥 = 𝑥}
  = id↦-intro 𝑥 ⊥ᵥ (NbhSys.⊑-⊥ (ValNbhSys _))

id↦-↓closed : ⊑ᵥ Γ 𝑦 𝑧 → 𝑥 id↦ 𝑧 → 𝑥 id↦ 𝑦
id↦-↓closed {𝑦 = 𝑦} {𝑥 = 𝑥} 𝑦⊑𝑧 (id↦-intro _ _ 𝑧⊑𝑥)
  = id↦-intro 𝑥 𝑦 (NbhSys.⊑-trans (ValNbhSys _) 𝑦⊑𝑧 𝑧⊑𝑥)

id↦-↑directed : 𝑥 id↦ 𝑦 → 𝑥 id↦ 𝑧 → (con : ValCon Γ 𝑦 𝑧) →
                𝑥 id↦ (𝑦 ⊔ᵥ 𝑧 [ con ])
id↦-↑directed {𝑥 = 𝑥} {𝑦} {𝑧} (id↦-intro _ _ 𝑦⊑𝑥)
  (id↦-intro _ _ 𝑧⊑𝑥) con
  = id↦-intro 𝑥 (𝑦 ⊔ᵥ 𝑧 [ con ]) (NbhSys.⊑-⊔ (ValNbhSys _) 𝑦⊑𝑥 𝑧⊑𝑥 con)

id↦-con : 𝑥 id↦ 𝑧 → 𝑦 id↦ 𝑤 → ValCon Γ 𝑥 𝑦 → ValCon Γ 𝑧 𝑤
id↦-con (id↦-intro _ _ 𝑧⊑𝑥) (id↦-intro _ _ 𝑤⊑𝑦) con
  = Con-⊔ᵥ 𝑧⊑𝑥⊔𝑦 𝑤⊑𝑥⊔𝑦
