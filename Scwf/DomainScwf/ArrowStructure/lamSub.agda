...
lamSubLemma₁' : ∀ 𝑥 𝑓 → [ lam 𝑡 ∘ γ ] 𝑥 ↦ ⟪ 𝐹 𝑓 ⟫ →
                ∀ x y → < x , y > ∈ 𝑓 →
                [ 𝑡 ∘ ⟨ γ ∘ (p Δ 𝐴) , q Δ 𝐴 ⟩ ] ⟪ x , 𝑥 ⟫ ↦ ⟪ y ⟫
lamSubLemma₁' {Δ = Δ} 𝑥 _
  (∘↦-intro _ 𝑦 _ γ𝑥↦𝑦 (lam↦-intro₂ _ 𝑓' p)) x y xy∈𝑓
...
lamSubLemma₁ 𝑥 ⟪ 𝐹 𝑓 , ⟪⟫ ⟫ (∘↦-intro _ 𝑓' _ γ𝑥↦𝑓' lam𝑓'↦𝑓)
  = lam↦-intro₂ 𝑥 𝑓 (lamSubLemma₁' 𝑥 𝑓 lam𝑥↦𝑓)
  where lam𝑥↦𝑓 = ∘↦-intro 𝑥 𝑓' ⟪ 𝐹 𝑓 ⟫ γ𝑥↦𝑓' lam𝑓'↦𝑓
...
getP-Struct' : {γ : tAppmap Δ Γ} →
               ...
               ∀ x′ y′ → < x′ , y′ > ∈ (< x , y > ∷ 𝑓) →
               [ 𝑡 ] ⟪ x′ , 𝑦 ⊔ᵥ 𝑧 ⟫ ↦ ⟪ y′ ⟫
getP-Struct' {Γ = Γ} {𝑡 = 𝑡} 𝑥 x y 𝑦 𝑧 𝑓 _ 𝑡x𝑦↦y _ _ _ here
  = Appmap.↦-mono 𝑡 x𝑦⊑x⊔ 𝑡x𝑦↦y
  where 𝑦⊑⊔ = NbhSys.⊑-⊔-fst (ValNbhSys _)
        x𝑦⊑x⊔ = ⊑ᵥ-cons (𝐴 :: Γ) ⟪ x , 𝑦 ⟫ ⟪ x , 𝑦 ⊔ᵥ 𝑧 ⟫
                (NbhSys.⊑-refl 𝐴) 𝑦⊑⊔
getP-Struct' {Γ = Γ} {𝑡 = 𝑡} 𝑥 x y 𝑦 𝑧 𝑓 _ _ p x′ y′
  (there x′y′∈𝑓)
  = Appmap.↦-mono 𝑡 x′r⊑x′⊔ (p x′ y′ x′y′∈𝑓)
  where r⊑⊔ = NbhSys.⊑-⊔-snd (ValNbhSys _)
        x′r⊑x′⊔ = ⊑ᵥ-cons (𝐴 :: Γ) ⟪ x′ , 𝑧 ⟫
                  ⟪ x′ , 𝑦 ⊔ᵥ 𝑧 ⟫ (NbhSys.⊑-refl 𝐴) r⊑⊔

getP-Struct : {γ : tAppmap Δ Γ} →
              ∀ 𝑥 → (𝑓 : NbhFinFun 𝐴 𝐵) →
              [ 𝑡 ∘ ⟨ γ ∘ p Δ 𝐴 , q Δ 𝐴 ⟩ ] 𝑥 lam↦ ⟪ 𝐹 𝑓 ⟫ →
              P-Struct γ 𝑡 𝑥 𝑓
...
getP-Struct 𝑥 (< x , y > ∷ 𝑓) (lam↦-intro₂ _ _ p)
  with (p x y here)
...
lamSubLemma₂ 𝑥 _ (lam↦-intro₂ _ 𝑓 p)
  with (getP-Struct 𝑥 𝑓 (lam↦-intro₂ 𝑥 𝑓 p))
lamSubLemma₂ {𝑡 = 𝑡} {γ = γ} 𝑥 _ (lam↦-intro₂ _ 𝑓 p)
  | record { 𝑦 = 𝑦
           ; γ𝑥↦𝑦 = γ𝑥↦𝑦
           ; λ𝑡𝑦 = λ𝑡𝑦
           }
  = ∘↦-intro 𝑥 𝑦 ⟪ 𝐹 𝑓 ⟫ γ𝑥↦𝑦 (lam↦-intro₂ 𝑦 𝑓 λ𝑡𝑦)
