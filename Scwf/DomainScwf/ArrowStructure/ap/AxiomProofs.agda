...
ap↦-mono {𝑥} {𝑦} 𝑥⊑𝑦 (ap↦-intro₂ x y 𝑓 𝑡𝑥↦𝑓 𝑢𝑥↦x xy⊑𝑓)
...
ap↦-↓closed' : ∀ {𝑓 x y y′} → [ 𝐵 ] y′ ⊑ y →
               [ ArrNbhSys 𝐴 𝐵 ] 𝐹 (< x , y > ∷ ∅) ⊑ 𝐹 𝑓 →
               ∀ x″ y″ → < x″ , y″ > ∈ (< x , y′ > ∷ ∅) →
               ⊑ₑ-proof 𝑓 x″ y″
ap↦-↓closed' {x = x} {y} {y′} y′⊑y (⊑ₑ-intro₂ _ _ p) _ _ here
  = record { sub = sub
...
ap↦-↓closed {𝑦 = ⟪ y , ⟪⟫ ⟫}
  ...
  = ap↦-intro₂ x′ y 𝑓 𝑡𝑥↦𝑓 𝑢𝑥↦x′ x′y⊑𝑓
  where x′y⊑𝑓' = ap↦-↓closed' y⊑y′ x′y′⊑𝑓′
        x′y⊑𝑓 = ⊑ₑ-intro₂ (< x′ , y > ∷ ∅) 𝑓 x′y⊑𝑓'
