...
apSubLemma₁ {𝑡 = 𝑡} {γ = γ} {𝑢} 𝑥 ⟪ y , ⟪⟫ ⟫ (ap↦-intro₁ p)
  ...
  where tupy⊑⊥ = ⊑ᵥ-cons [ 𝐵 ] ⟪ y ⟫ ⊥ᵥ p ⊑ᵥ-nil
        ap𝑡𝑢∘γ↦⊥ = Appmap.↦-bottom (ap 𝑡 𝑢 ∘ γ)
apSubLemma₁ {𝑡 = 𝑡} {γ = γ} {𝑢} 𝑥 _
  (ap↦-intro₂ x y 𝑓 (∘↦-intro _ 𝑦 _ γ𝑥↦𝑦 𝑡𝑦↦𝑓)
  (∘↦-intro _ 𝑧 _ γ𝑥↦𝑧 𝑢𝑧↦x) xy⊑𝑓)
  = ∘↦-intro 𝑥 (𝑦 ⊔ᵥ 𝑧) ⟪ y ⟫ γ𝑥↦𝑦⊔𝑧 ap𝑡𝑢𝑦𝑧↦y
  where γ𝑥↦𝑦⊔𝑧 = Appmap.↦-↑directed γ γ𝑥↦𝑦 γ𝑥↦𝑧
        𝑡𝑦𝑧↦𝑓 = Appmap.↦-mono 𝑡 (NbhSys.⊑-⊔-fst (ValNbhSys _)) 𝑡𝑦↦𝑓
        𝑢𝑦𝑧↦x = Appmap.↦-mono 𝑢 (NbhSys.⊑-⊔-snd (ValNbhSys _)) 𝑢𝑧↦x
        ap𝑡𝑢𝑦𝑧↦y = ap↦-intro₂ x y 𝑓 𝑡𝑦𝑧↦𝑓 𝑢𝑦𝑧↦x xy⊑𝑓
...
apSubLemma₂ 𝑥 _ (∘↦-intro _ 𝑧 _ γ𝑥↦𝑧
  (ap↦-intro₂ x y 𝑓 𝑡𝑧↦𝑓 𝑢𝑧↦x xy⊑𝑓))
  = ap↦-intro₂ x y 𝑓 𝑡∘γ𝑥↦𝑓 𝑢∘γ𝑥↦x xy⊑𝑓
  where 𝑡∘γ𝑥↦𝑓 = ∘↦-intro 𝑥 𝑧 ⟪ 𝐹 𝑓 ⟫ γ𝑥↦𝑧 𝑡𝑧↦𝑓
