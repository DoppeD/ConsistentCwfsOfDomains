...
Con-⊔ₓ {< x₁ , x₂ >} {< z₁ , z₂ >} {< y₁ , y₂ >}
  (⊑ₓ-intro₂ _ _ _ _ x₁⊑z₁ x₂⊑z₂) (⊑ₓ-intro₂ _ _ _ _ y₁⊑z₁ y₂⊑z₂)
  = con-pair (NbhSys.Con-⊔ 𝒟 x₁⊑z₁ y₁⊑z₁) (NbhSys.Con-⊔ 𝒟′ x₂⊑z₂ y₂⊑z₂)

⊑ₓ-refl : x ⊑ₓ x
⊑ₓ-refl {x = ⊥ₓ} = ⊑ₓ-intro₁
⊑ₓ-refl {x = < x₁ , x₂ >}
  = ⊑ₓ-intro₂ x₁ x₁ x₂ x₂ (NbhSys.⊑-refl 𝒟) (NbhSys.⊑-refl 𝒟′)

⊑ₓ-trans : x ⊑ₓ y → y ⊑ₓ z → x ⊑ₓ z
⊑ₓ-trans {z = z} ⊑ₓ-intro₁ ⊑ₓ-intro₁ = ⊑ₓ-intro₁
⊑ₓ-trans ⊑ₓ-intro₁ (⊑ₓ-intro₂ _ y₁ _ y₂ _ _) = ⊑ₓ-intro₁
⊑ₓ-trans (⊑ₓ-intro₂ x₁ y₁ x₂ y₂ x₁⊑y₁ x₂⊑y₂)
  (⊑ₓ-intro₂ _ z₁ _ z₂ y₁⊑z₁ y₂⊑z₂)
  = ⊑ₓ-intro₂ x₁ z₁ x₂ z₂
...
⊑ₓ-⊔ {< y₁ , y₂ >} {x} {⊥ₓ} y⊑x _ - = y⊑x
⊑ₓ-⊔ {< y₁ , y₂ >} {x} {< z₁ , z₂ >}
  (⊑ₓ-intro₂ _ w₁ _ w₂ y₁⊑w₁ y₂⊑w₂)
  (⊑ₓ-intro₂ _ _ _ _ z₁⊑w₁ z₂⊑w₂) (con-pair cony₁z₁ cony₂z₂)
  = ⊑ₓ-intro₂ ([ 𝒟 ] y₁ ⊔ z₁ [ cony₁z₁ ]) w₁
    ([ 𝒟′ ] y₂ ⊔ z₂ [ cony₂z₂ ]) w₂ y₁⊔z₁⊑w₁ y₂⊔z₂⊑w₂
...
⊑ₓ-⊔-fst {⊥ₓ} {_} _ = ⊑ₓ-intro₁
⊑ₓ-⊔-fst {< x₁ , y₁ >} {⊥ₓ} _ =
  ⊑ₓ-intro₂ x₁ x₁ y₁ y₁ (NbhSys.⊑-refl 𝒟) ((NbhSys.⊑-refl 𝒟′))
⊑ₓ-⊔-fst {< x₁ , y₁ >} {< x₂ , y₂ >} (con-pair conx₁x₂ cony₁y₂) =
  ⊑ₓ-intro₂ x₁ ([ 𝒟 ] x₁ ⊔ x₂ [ conx₁x₂ ]) y₁ ([ 𝒟′ ] y₁ ⊔ y₂ [ cony₁y₂ ])
  x₁⊑x₁⊔x₂ y₁⊑y₁⊔y₂
