...
p↦-mono (⊑ᵥ-cons _ _ 𝑥⊑𝑦) (p↦-intro 𝑧⊑𝑥)
  = p↦-intro 𝑧⊑tail𝑦
  where 𝑧⊑tail𝑦 = NbhSys.⊑-trans (ValNbhSys _) 𝑧⊑𝑥 𝑥⊑𝑦

p↦-bottom : {𝑥 : Valuation (𝐴 :: Γ)} → 𝑥 p↦ ⊥ᵥ
p↦-bottom {𝑥 = 𝑥} = p↦-intro (NbhSys.⊑-⊥ (ValNbhSys _))

p↦-↓closed : {𝑥 : Valuation (𝐴 :: Γ)} → ∀ {𝑦 𝑧} →
             ⊑ᵥ Γ 𝑦 𝑧 → 𝑥 p↦ 𝑧 → 𝑥 p↦ 𝑦
p↦-↓closed 𝑦⊑𝑧 (p↦-intro 𝑧⊑𝑥)
  = p↦-intro (NbhSys.⊑-trans (ValNbhSys _) 𝑦⊑𝑧 𝑧⊑𝑥)

p↦-↑directed : {𝑥 : Valuation (𝐴 :: Γ)} → ∀ {𝑦 𝑧} →
               𝑥 p↦ 𝑦 → 𝑥 p↦ 𝑧 →
               (con𝑦𝑧 : ValCon _ 𝑦 𝑧) →
               𝑥 p↦ (𝑦 ⊔ᵥ 𝑧 [ con𝑦𝑧 ])
p↦-↑directed (p↦-intro 𝑦⊑𝑥) (p↦-intro 𝑧⊑𝑥) con𝑦𝑧
  = p↦-intro 𝑦⊔𝑧⊑tail𝑥
...
p↦-con (p↦-intro 𝑦⊑𝑥) (p↦-intro 𝑦′⊑𝑥′) (con-tup _ con𝑥𝑥′)
  = Con-⊔ᵥ 𝑦⊑𝑥⊔𝑥′ 𝑦′⊑𝑥⊔𝑥′
