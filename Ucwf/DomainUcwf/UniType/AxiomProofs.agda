...
⊑ᵤ-⊔ᵤ : y ⊑ᵤ x → z ⊑ᵤ x → (y ⊔ᵤ z) ⊑ᵤ x
⊑ᵤ-⊔ᵤ {⊥ᵤ} {x} {⊥ᵤ} _ _ = ⊑ᵤ-intro₁
⊑ᵤ-⊔ᵤ {λᵤ 𝑓} {x} {⊥ᵤ} y⊑x _ = y⊑x
⊑ᵤ-⊔ᵤ {⊥ᵤ} {x} {λᵤ 𝑓} _ z⊑x = z⊑x
⊑ᵤ-⊔ᵤ {λᵤ 𝑓′} {λᵤ 𝑓} {λᵤ 𝑓″} y⊑x z⊑x
  = ⊑ᵤ-intro₂ (𝑓′ ∪ₛ 𝑓″) 𝑓 (⊑ᵤ-⊔ᵤ' y⊑x z⊑x)

⊑ᵤ-⊔ᵤ-helper₁ : ∀ {x} → x ⊑ᵤ (x ⊔ᵤ ⊥ᵤ)
⊑ᵤ-⊔ᵤ-helper₁ {x} rewrite (⊔ᵤ-⊥ᵤ-rightid x)
  = ⊑ᵤ-refl

⊑ᵤ-⊔ᵤ-helper₂ : ∀ {x} →  (x ⊔ᵤ ⊥ᵤ) ⊑ᵤ x
⊑ᵤ-⊔ᵤ-helper₂ {x} rewrite (⊔ᵤ-⊥ᵤ-rightid x)
  = ⊑ᵤ-refl

⊑ᵤ-⊔ᵤ-fst : ∀ {x y} → x ⊑ᵤ (x ⊔ᵤ y)
⊑ᵤ-⊔ᵤ-fst {⊥ᵤ} = ⊑ᵤ-intro₁
⊑ᵤ-⊔ᵤ-fst {λᵤ 𝑓} {⊥ᵤ} = ⊑ᵤ-refl
⊑ᵤ-⊔ᵤ-fst {λᵤ 𝑓} {λᵤ 𝑓′}
...
⊑ᵤ-⊔ᵤ-snd : ∀ {x y} → y ⊑ᵤ (x ⊔ᵤ y)
⊑ᵤ-⊔ᵤ-snd {y = ⊥ᵤ} = ⊑ᵤ-intro₁
⊑ᵤ-⊔ᵤ-snd {⊥ᵤ} {λᵤ 𝑓} = ⊑ᵤ-refl
⊑ᵤ-⊔ᵤ-snd {λᵤ 𝑓} {λᵤ 𝑓′}
  = ⊑ᵤ-intro₂ 𝑓′ (𝑓 ∪ₛ 𝑓′) λ x y xy∈𝑓′ →
    record { sub = < x , y >ₛ ∷ ∅
           ; y⊑ᵤpost = ⊑ᵤ-⊔ᵤ-helper₁
