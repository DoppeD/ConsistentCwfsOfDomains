...
β-lemma₁ : ∀ {𝑥 y} →
           [ ap (lam 𝑢) 𝑡 ] 𝑥 ↦ y →
           [ 𝑢 ∘ ⟨ idMap (nToCtx m) , 𝑡 ⟩ ] 𝑥 ↦ y
β-lemma₁ {m = m} {𝑢 = 𝑢} {𝑡 = 𝑡} {y = ⊥ᵤ} _
  = ∘↦-intro ⟨⟩⊥↦⊥ 𝑢⊥↦⊥
  where id⊥↦⊥ = Appmap.↦-bottom (idMap (nToCtx m))
        𝑡𝑥↦⊥ = Appmap.↦-bottom 𝑡
        ⟨⟩⊥↦⊥ = ⟨⟩↦-intro {𝑦 = ⟪ _ ,, ⊥ᵥ ⟫} id⊥↦⊥ 𝑡𝑥↦⊥
        𝑢⊥↦⊥ = Appmap.↦-bottom 𝑢
β-lemma₁ {y = λᵤ 𝑓} (ap↦-intro₂ _ _ (⊑ᵤ-intro₂ _ _ p))
  with (p _ _ here)
β-lemma₁ {m = m} {𝑢 = 𝑢} {𝑡 = 𝑡} {y = λᵤ 𝑓}
  ...
  where id𝑥↦𝑥 = id↦-intro (NbhSys.⊑-refl (ValNbhSys _))
        ...
        𝑢x𝑥↦y = Appmap.↦-↓closed 𝑢 y⊑ᵤpost 𝑢x𝑥↦post
...
β-lemma₂ : ∀ {𝑥 y} →
           [ 𝑢 ∘ ⟨ idMap (nToCtx n) , 𝑡 ⟩ ] 𝑥 ↦ y →
           [ ap (lam 𝑢) 𝑡 ] 𝑥 ↦ y
β-lemma₂ {y = ⊥ᵤ} _ = ap↦-intro₁
β-lemma₂ {n = n} {𝑢 = 𝑢} {y = λᵤ 𝑓}
