...
apSubLemma₁ : ∀ 𝑥 𝑦 → [ ap (𝑡 ∘ γ) (𝑢 ∘ γ) ] 𝑥 ↦ 𝑦 →
              [ ap 𝑡 𝑢 ∘ γ ] 𝑥 ↦ 𝑦
apSubLemma₁ {γ = γ} 𝑥 ⟪ ⊥ᵤ , _ ⟫ ap↦-intro₁ =
  ∘↦-intro 𝑥 ⊥ᵥ ⟪ ⊥ᵤ ⟫ (Appmap.↦-bottom γ) ap↦-intro₁
apSubLemma₁ {γ = γ} 𝑥 ⟪ ⊥ᵤ , _ ⟫ (ap↦-intro₂ x y 𝑓
  (∘↦-intro _ 𝑦 _ γ𝑥↦𝑦 𝑡𝑦↦𝑓) (∘↦-intro _ 𝑧 _ γ𝑥↦𝑧 𝑢𝑧↦x) xy⊑𝑓)
  = ∘↦-intro 𝑥 ⊥ᵥ ⟪ ⊥ᵤ ⟫ (Appmap.↦-bottom γ) ap↦-intro₁
apSubLemma₁ {𝑡 = 𝑡} {γ = γ} {𝑢 = 𝑢} 𝑥 ⟪ λᵤ 𝑓 , _ ⟫
  (ap↦-intro₂ x y 𝑔 (∘↦-intro _ 𝑦 _ γ𝑥↦𝑦 𝑡𝑦↦𝑔)
  (∘↦-intro _ 𝑧 _ γ𝑥↦𝑧 𝑢𝑧↦x) xy⊑𝑔)
  = ∘↦-intro 𝑥 (𝑦 ⊔ᵥ 𝑧) ⟪ y ⟫ γ𝑥↦𝑦⊔𝑧 ap𝑡𝑢𝑦𝑧↦y
  where 𝑦⊑𝑦⊔𝑧 = NbhSys.⊑-⊔-fst (ValNbhSys _)
        𝑡𝑦𝑧↦𝑔 = Appmap.↦-mono 𝑡 𝑦⊑𝑦⊔𝑧 𝑡𝑦↦𝑔
        𝑧⊑𝑦⊔𝑧 = NbhSys.⊑-⊔-snd (ValNbhSys _)
        𝑢𝑦𝑧↦x = Appmap.↦-mono 𝑢 𝑧⊑𝑦⊔𝑧 𝑢𝑧↦x
        ap𝑡𝑢𝑦𝑧↦y = ap↦-intro₂ x (λᵤ 𝑓) 𝑔 𝑡𝑦𝑧↦𝑔 𝑢𝑦𝑧↦x xy⊑𝑔
        γ𝑥↦𝑦⊔𝑧 = Appmap.↦-↑directed γ γ𝑥↦𝑦 γ𝑥↦𝑧

apSubLemma₂ : ∀ 𝑥 𝑦 → [ ap 𝑡 𝑢 ∘ γ ] 𝑥 ↦ 𝑦 →
              [ ap (𝑡 ∘ γ) (𝑢 ∘ γ) ] 𝑥 ↦ 𝑦
apSubLemma₂ 𝑥 ⟪ ⊥ᵤ , ⟪⟫ ⟫ _ = ap↦-intro₁
apSubLemma₂ 𝑥 ⟪ λᵤ 𝑓 , ⟪⟫ ⟫
  (∘↦-intro _ 𝑧 _ γ𝑥↦𝑧 (ap↦-intro₂ x _ 𝑔 𝑡𝑧↦𝑔 𝑢𝑧↦x x𝑓⊑𝑔))
  = ap↦-intro₂ x (λᵤ 𝑓) 𝑔 𝑡∘γ𝑥↦𝑓 𝑢∘γ𝑥↦x x𝑓⊑𝑔
  where 𝑡∘γ𝑥↦𝑓 = ∘↦-intro 𝑥 𝑧 ⟪ λᵤ 𝑔 ⟫ γ𝑥↦𝑧 𝑡𝑧↦𝑔
        𝑢∘γ𝑥↦x = ∘↦-intro 𝑥 𝑧 ⟪ x ⟫ γ𝑥↦𝑧 𝑢𝑧↦x
